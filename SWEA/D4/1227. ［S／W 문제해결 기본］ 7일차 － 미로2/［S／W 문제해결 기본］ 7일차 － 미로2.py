...
for t in range(1, 11):
    ...
    maze = [list(map(int, input())) for _ in range(100)]      # 미로 배열 받아오기
    delta = [[-1, 0], [1, 0], [0, -1], [0, 1]]      # 상하좌우 탐색할 델타리스트 생성
    visit = [[False] * 100 for _ in range(100)]       # 방문 기록 리스트 생성
    stack = []
    res = 0

    for i in range(100):
        for j in range(100):                 # 전체 배열을 돌면서
    ...
    while stack:                # 스택이 차있는 동안 반복 (스택이 비어있다면 종료)
        ...
        for x, y in delta:      # 탐색할 방향 설정
            dx = ix + x
            dy = iy + y
            if 0 <= dx < 100 and 0 <= dy < 100:   # 미로 범위를 벗어나지 않고
